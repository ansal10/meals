...
const applyCaloriesData = async (user, meals) => {
    ...
    let dailyDatesData = await models.Meal.findAll({
        where: {
            date: {
                [Op.in]: dates
            }
    ...
    let dailyDatesDataObj = {};
    dailyDatesData.forEach((d) => {
        dailyDatesDataObj[d.dataValues.date] = d.dataValues.dayCalories;
    });
    ...
    if (user.role === 'manager' || user.role === 'admin') {
        let userData = await models.User.findAll({
            ...
            }, attributes: ['id', 'calorieGoal']
        });

        for (let i = 0; i < userData.length; i++) {
            userDataObj[userData[i].dataValues.id] = Number(userData[i].dataValues.calorieGoal);
        }
    } else {
        userDataObj[user.id] = user.calorieGoal;
    }

    for (let i = 0; i < meals.length; i++) {
        meals[i] = meals[i].dataValues;
        meals[i].edit = true;
        meals[i].dailyCalorieIntake = dailyDatesDataObj[meals[i].date];
        meals[i].dailyyCalorieGoal = userDataObj[meals[i].UserId];
    }
    return meals;
};
